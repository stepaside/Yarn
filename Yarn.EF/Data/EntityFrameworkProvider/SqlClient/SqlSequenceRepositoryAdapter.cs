...
namespace Yarn.Data.EntityFrameworkProvider.SqlClient
{
    public class SqlSequenceRepositoryAdapter : RepositoryAdapter
    {
        ...
        public SqlSequenceRepositoryAdapter(IRepository repository, ISequenceNameProvider provider, bool autoRestart = false, int rangeSize = 10)
            : base(repository)
        {
            if (!(((IDataContext<DbContext>)Repository.DataContext).Session.Database.Connection is SqlConnection))
            {
        ...
        public override T Add<T>(T entity)
        {
            ...
            if (primaryKey.Length == 1)
            {
                ...
                var value = GenerateId(sequence, property.PropertyType, (SqlConnection)((IDataContext<DbContext>)Repository.DataContext).Session.Database.Connection);
